...
void initVarStore(HashMap* map) {
    ...
    StringView* viewTrue = newStringView("true", 4);
    Number* numTrue = numNew(NUM_REAL);
    mpfr_set_ui(numTrue->real, 1, MPFR_RNDN);
    insertVar(map, viewTrue, numTrue);

    StringView* viewFalse = newStringView("false", 5);
    Number* numFalse = numNew(NUM_REAL);
    mpfr_set_ui(numFalse->real, 0, MPFR_RNDN);
    insertVar(map, viewFalse, numFalse);
}
